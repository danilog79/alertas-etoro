...
class Scanner:
    ...
    def scan_once(self):
        ...
        scanned = 0
        intraday_atr_pcts = []
        universe_intraday = list(dict.fromkeys(self.config["watchlists"].get("intraday", []) + self.config["watchlists"].get("etoro_featured_winners", []) + self.config["watchlists"].get("etoro_featured_losers", []) + self.regime.get("etoro_recommendations", [])))
        universe_swing = list(dict.fromkeys(self.config["watchlists"].get("swing", []) + self.config["watchlists"].get("etoro_featured_winners", []) + self.config["watchlists"].get("etoro_featured_losers", [])))
